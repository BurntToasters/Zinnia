...
| :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------- |
| **EXE: [x64](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-Windows-x64.exe) / [arm64](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-Windows-arm64.exe)** | **[Universal DMG](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-macOS.dmg)**   | **AppImage:** [x64](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-Linux-x64.AppImage) |
| <!-- <div align="center"><a href="https://apps.microsoft.com/detail/9pkgd6lkcl5j?referrer=appbadge&mode=full"><img src="https://get.microsoft.com/images/en-us%20light.svg" width="150"/></a></div>-->                     | **[Universal ZIP](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-macOS.zip)**   | **DEB:** [x64](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-Linux-x64.deb)           |
|                                                                                                                                                                                                                            |                                                                                                                 | **RPM:** [x64](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-Linux-x64.rpm)           |
|                                                                                                                                                                                                                            |                                                                                                                 | **Flatpak:** [x64](https://github.com/BurntToasters/Zinnia/releases/download/v0.6.1-beta.9/Zinnia-Linux-x64.flatpak)   |
...
- **Fix:** Quit after a kill error still `app.exit(0)`. Prepare-task panics release the slot. Extract-window probe no longer takes the global 7-Zip slot. Warm-idle re-checks generation before exit. Recovery Dismiss keeps Acknowledge while the journal exists.
- **Windows:** Shell DLLs link `/MT`. NSIS `/UPDATE` never Aborts on leftover AppX; missing register script probes packages instead of failing closed. Classic Extract fallback covers `.rar` `.tgz` `.tbz2` `.txz` `.001`.
...
- **macOS:** Finder Sync monitors `mountedVolumeURLs` and remounts, not the `/Volumes` folder itself.
- **UI:** Settings Save copies live Power format/method fields. Workspace switch to Power no longer wipes extras. Batch extract unfreezes Basic destination/password. Password prompts ignore overlay clicks. Extract Cancel stays usable during the prompt. Wizard/settings sit above the Basic header. Sticky toasts are keyboard-dismissible. Extra-args placeholder is `-y -bb1 -bsp1`.
- **Docs:** `docs/RELEASE-STABLE.md` exists. `SECURITY.md` lists `code@rosie.run`. AppImage FUSE 2 is on the Linux QA gate.
- **Fix:** Extract-only warm-idle now exits after idle: the timer no longer invalidates its own generation when tearing down the tray. Startup recovery marks itself done after a panic, and `run_7z` fails closed after 120s if recovery is still walking a journal. Quit retries a stuck 7-Zip kill once before `exit(0)`.
- **Fix:** Rust extract destinations match the frontend: `.` / `..` / all-dot / trailing-dot-or-space stems become `_extracted`. Exclusive-copy MotW/quarantine stamp failures delete the partial dest. ZIP add emits `-mcu=on`; extra-args `-mcu=off` is rejected and the backend last-wins `-mcu=on` on ZIP `a`/`u`. Caller `-r` / `-scs*` / `-scc*` are rejected in Rust, not only in extra-args UI.
- **Fix:** Settings Save writes Basic live compress fields. Dismissing the main password prompt cancels the batch. Integrity test treats remapped `warning_code` as passed-with-warnings, not a clean pass. Browse/test/convert/Basic password probes use the guarded `run_7z` wrapper.
- **UI:** Basic licenses sit above Settings (`10051`). Sticky toasts and the recovery banner stay above the wizard and are not marked `inert` while a modal is open.
- **macOS:** Services Extract includes split volumes (`.001` via `run.rosie.zinnia.split-volume`). QA classifies Tahoe Finder Sync host-death vs a Zinnia misconfiguration.

## Changes in `v0.6.1-beta.8:`

- **Release:** Post-tag follow-up to published `v0.6.1-beta.7`. Does not retag that GitHub prerelease.
- **Fix:** Extract-window capability closure. The compress-input probe moved to its own IPC module so the extract window no longer imports `probe_compress_inputs`; the transitive permissions test is green again.
- **Fix:** Release signing detects gh's real `(HTTP 422)` conflict text (via parsed status), so stable asset re-uploads, the beta sync-lock retry, and duplicate-asset replacement work instead of re-POSTing multi-hundred-MB files three times.
- **Fix:** Draft reuse re-asserts the `prerelease` flag; a stale flag could hide stable 0.6.1 from `/releases/latest` forever. Duplicate drafts now abort gpg-sign and verify-draft instead of picking one. `release:verify:draft` resolves every manifest download URL against the draft assets (payload + `.sig`), refusing feeds that point at 404s. New `npm run release:publish` publishes only after verify:draft passes. The 30-minute draft wait tolerates transient GitHub errors.
- **Fix:** release preparation invokes `test:all -- --require-clean-proof`, so a dirty tree cannot silently skip quality-gate proof; ordinary `test:all` remains usable for local dirty worktrees.
- **Fix:** Stable release policy refuses `ENFORCE_LINUX_X64_PACKAGE_SET=0` and non-canonical `GH_REPO_OWNER` / `GH_REPO_NAME` / `RELEASE_DOWNLOAD_BASE_URL` overrides. CHANGELOG placeholder notes fail validation on beta tags too.
- **Fix:** Failed-kill slot wedge: a 7-Zip child retained after a kill error is now reaped once it exits, freeing the shared slot; a journal-marker unlink failure after completed finalization no longer wedges it either.
- **Fix:** A crash between the sibling whole-stage rename and its commit marker no longer permanently locks every archive operation: a user-confirmed banner action accepts the kept destination and clears only the recovery journal (new `acknowledge_preserved_transaction`).
- **Security:** Windows member-path validation now rejects trailing-dot/space parent forms (`.. `), NTFS alternate-data-stream colons, and reserved device names (`CON`, `aux.c`, ...).
- **Fix:** Exit-1 metadata-only classification catches 7-Zip's plural `Errors:` / system-error summary lines; `-bsp2` (progress into stderr, spoofing the classifier) is no longer an accepted extra arg, UI allow-list matches `validation.rs` per command.
- **Fix:** `archive_output_selection_token` hashes off the main thread; update reservations use a rolling heartbeat TTL while install is live; lost reserve IPC responses release the orphan; channel changes cannot close an in-flight install.
- **Fix:** Basic-mode compress resets Power-only `store-timestamps`; batch extract freezes destination/password/extra-args inputs mid-run; settings modal clamps auto-close values; password redaction fails closed to end-of-line like the frontend, and run_7z scrubs parent-side argument copies right after spawn so a password does not linger through finalization.
- **Hardening:** Publish-target chmod is fd-bound (`O_NOFOLLOW` + `fchmod`); Windows directory sync distinguishes open failures from flush failures; shared-temp 7-Zip list-dir sweep skips other users' files; log trim reads a bounded tail instead of the whole file; staging names must be single path components.
- **Tooling:** `.githooks` co-author stripping regains its executable bit; CI quality-gate installs `clippy`, pins `NPM_VERSION=12.0.2`; Flatpak build passes `--engine-strict=false` to `npm ci`; dropped dead `updater.active` config key.
- **Docs:** `docs/RELEASE-STABLE.md` runbook for the beta.8-to-0.6.1 transition.
- **Fix:** Windows archive fixtures list with `-sccUTF-8` so `unicode.zip` members match the app's `harden_7z_args` path.
- **Fix:** Explorer/Finder extracts queue onto main when the shared 7-Zip slot is already held, not only when an extract window is open.
- **Fix:** Main bootstrap tests await the actual startup promise instead of a timing heuristic, removing Linux/macOS release-gate flakes. E2E teardown asks Tauri to exit cleanly on every OS, preventing orphaned Windows WebView2 handles, and fresh E2E builds restore tracked ACL schemas so the clean-tree release proof stays valid. Windows profile cleanup also retries transient handle locks without failing an otherwise completed suite. Full beta release commands accept `--skip-check` to reuse and sign against an existing Windows-created draft after the release branch advances, while retaining every other release gate. The macOS SSH wrapper forwards release flags directly instead of letting nested npm parsing reject them. Draft creation, reuse, signing, and verification now fail immediately when GitHub returns a same-version draft with an `untagged-*` placeholder instead of waiting or uploading against ambiguous metadata.
- **Fix:** Extra-args reject 7-Zip include/exclude list expansion (`-i!` / `-x!`) and method values with path separators. Invalid custom-preset fields drop the preset instead of coercing it to defaults under the same name.
- **Fix:** Extract windows disable auto-close if settings fail to load. Interactive updater checks skip Flatpak. `ZINNIA_E2E` must be `1`. `SKIP_E2E=1` fails the quality gate.
- **Windows:** Sparse-payload `zinnia.exe` lookup ignores a sibling plant under `shell-*`. Registration checks Authenticode on DLLs and MSIX, restores the newest previous payload by parsed version, and exits 0 after a successful restore so NSIS does not stack classic verbs.
- **Linux:** Archive-default helpers run `/usr/bin/xdg-mime` or `/bin/xdg-mime`, not `PATH`.
- **Docs:** Finder Sync folder scope, main-window extract destination privilege, and fail-closed E2E skip policy.
