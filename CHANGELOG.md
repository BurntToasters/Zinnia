...
- **Fix:** Main bootstrap tests await the actual startup promise instead of a timing heuristic, removing Linux/macOS release-gate flakes. Windows E2E profile cleanup retries transient handle locks without failing an otherwise completed suite. Full beta release commands accept `--skip-check` to reuse and sign against an existing Windows-created draft after the release branch advances, while retaining every other release gate. The macOS SSH wrapper forwards release flags directly instead of letting nested npm parsing reject them.
